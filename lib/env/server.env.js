...
import { feedback, readWorkerFile } from '#utils';
...
function createServer(filePath) {
  return http.createServer(async (request, response) => {
    ...
    try {
      ...
      feedback.error('Failed to load code:', error);
...
async function startServer(filePath, port) {
  ...
  watcher.on('change', async () => {
    feedback.info('Code change detected. Restarting server...');
    // Close the current server
    server.close();
    // And create a new one
    server = createServer(filePath);
    server.listen(port, () => {
      feedback.success(`Server running on port ${port}`);
    });
  });
