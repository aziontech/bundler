...
async function buildToLocalServer() {
  // TODO: check the need to run the build command first because of the project configs that need to be in vulcan.config or .vulcan
  const { entry } = (await vulcan.readVulcanEnv('local')) || {};

  if (!entry) {
    // TODO: move msg to constants
    const msg =
      'please run the build command before running your project for the first time';
