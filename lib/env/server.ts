 ...
 */
async function manageServer(workerPath: string | null, port: number, skipProjectBuild = false) {
  try {
    if (currentServer) {
    ...
    const {
      setup: { entry },
    } = await buildCommand({ production: false, skipProjectBuild });
 ...
 */
async function startServer(workerPath: string | null, port: number, skipProjectBuild = false) {
  const IsPortInUse = await checkPortAvailability(port);
  if (IsPortInUse) {
    feedback.server.error(`Port ${port} is in use. Please choose another port.`);
    process.exit(1);
  }
  await manageServer(workerPath, port, skipProjectBuild);
