  ...
  for (const entry of entries) {
    ...
    if (extNames.includes(ext) && files.includes(`${entry.name.replace(ext, '')}`)) {
      ...
      if (!result.find((file) => file.name === fileName)) {
        result.push({ name: fileName, path: `${path.dirname(name)}${fileName}` });
      }
    }
    if (entry.isFile() && includeDirs.some((dir) => nameAsset.startsWith(normalizeDirPath(dir)))) {
      ...
      result.push({ name: normalizeDirPath(path.relative(rootDir, name)), path: name });
    }
  }
}
...
class BuildStatic {
  ...
  run = () => {
    ...
    fs.rmSync(path.resolve(this.config.rootDir, this.config.staticOutDir), { recursive: true, force: true });
    ...
    const staticFiles = FILES_ADDED.filter((file) =>
      this.config.staticDirs.some((stDirs) => file.name.startsWith(normalizeDirPath(stDirs)))
    );
    const serverFiles = FILES_ADDED.filter(
      (file) => !this.config.staticDirs.some((stDirs) => file.name.startsWith(normalizeDirPath(stDirs)))
    );
   ...
   \n`;
    ...
    for (const [index, file] of staticFiles.entries()) {
      const contentType = mime.lookup(file.name);
      assets += `${JSON.stringify(file.name)}: { contentType: "${contentType}", content: ${JSON.stringify(
