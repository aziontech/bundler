...
const constants_1 = require('next/constants.js');
const is_error_1 = __importDefault(require('next/dist/lib/is-error.js'));
const node_1 = require('next/dist/server/api-utils/node.js');
const base_server_1 = __importStar(require('next/dist/server/base-server.js'));
const request_meta_1 = require('next/dist/server/request-meta.js');
const render_1 = require('next/dist/server/render.js');
const send_payload_1 = require('next/dist/server/send-payload.js');
const server_route_utils_1 = require('next/dist/server/server-route-utils.js');
const normalize_page_path_1 = require('next/dist/shared/lib/page-path/normalize-page-path.js');
const path_match_1 = require('next/dist/shared/lib/router/utils/path-match.js');
const prepare_destination_1 = require('next/dist/shared/lib/router/utils/prepare-destination.js');
const utils_1 = require('next/dist/shared/lib/utils.js');
const compute_js_1 = require('./compute-js.js');
const require_1 = require('./require.js');
const load_components_1 = require('./load-components.js');
const serve_static_1 = require('./serve-static.js');
const body_streams_1 = require('next/dist/server/body-streams.js');
const utils_2 = require('next/dist/server/utils.js');
const response_cache_1 = __importDefault(
  require('next/dist/server/response-cache.js')
);
const incremental_cache_1 = require('next/dist/server/lib/incremental-cach.js');
const app_paths_1 = require('next/dist/shared/lib/router/utils/app-paths.js');
const get_route_from_asset_path_1 = __importDefault(
  require('next/dist/shared/lib/router/utils/get-route-from-asset-path.js')
);
const normalize_locale_path_1 = require('next/dist/shared/lib/i18n/normalize-locale-path.js');
const detect_domain_locale_1 = require('next/dist/shared/lib/i18n/detect-domain-locale.js');
const remove_trailing_slash_1 = require('next/dist/shared/lib/router/utils/remove-trailing-slash.js');
const utils_3 = require('next/dist/shared/lib/router/utils.js');
/**
...
class NextComputeJsServer extends base_server_1.default {
  ...
  getResponseCache({ dev }) {
    let _a;
    const incrementalCache = new incremental_cache_1.IncrementalCache({
      ...
      incrementalCacheHandlerPath:
        (_a = this.nextConfig.experimental) === null || _a === void 0
          ? void 0
          : _a.incrementalCacheHandlerPath,
    ...
    return new response_cache_1.default(incrementalCache, this.minimalMode);
  }
  ...
  getHasStaticDir() {
    return (0, require_1.assetDirectoryExists)(
      this.serverOptions.computeJs.assets,
      (0, path_1.join)(this.dir, 'static'),
      this.dir
    );
  }

  getPagesManifest() {
    const pagesManifestFile = (0, path_1.join)(
      this.serverDistDir,
      constants_1.PAGES_MANIFEST
    );
    return (0, require_1.readAssetManifest)(
      this.serverOptions.computeJs.assets,
      pagesManifestFile,
      this.dir
    );
  ...
  getAppPathsManifest() {
    if (this.nextConfig.experimental.appDir) {
      const appPathsManifestPath = (0, path_1.join)(
        this.serverDistDir,
        constants_1.APP_PATHS_MANIFEST
      );
      return (0, require_1.readAssetManifest)(
        this.serverOptions.computeJs.assets,
  ...
  async hasPage(pathname) {
    ...
    let found = false;
    try {
      found = !!this.getPagePath(
        pathname,
        (_a = this.nextConfig.i18n) === null || _a === void 0
          ? void 0
          : _a.locales
      );
    } catch (_) {}
  ...
  getBuildId() {
    const buildIdFile = (0, path_1.join)(
      this.distDir,
      constants_1.BUILD_ID_FILE
    );
    try {
      const content = (0, require_1.readAssetFileAsString)(
        this.serverOptions.computeJs.assets,
        buildIdFile,
        this.dir
      );
        ...
        fn: async (req, res, params, parsedUrl) => {
          ...
          if (publicFiles.has(path)) {
            await this.serveStatic(
              req,
              res,
              (0, path_1.join)(this.publicDir, ...pathParts),
              parsedUrl
            );
            return {
              finished: true,
  ...
  async proxyRequest(req, res, parsedUrl, upgradeHead) {
    ...
    const backend = (0, compute_js_1.getBackendInfo)(
  ...
  async runApi(req, res, query, params, page, builtPagePath) {
    ...
    const pageModule = await (0, require_1.readAssetModule)(
      this.serverOptions.computeJs.assets,
      builtPagePath,
      this.dir
    );
  ...
  async renderHTML(req, res, pathname, query, renderOpts) {
    ...
    /*
         ...
         */
    return (0, render_1.renderToHTML)(
      req.originalRequest,
      res.originalResponse,
      pathname,
      query,
      ...
      const buildRewrite = (rewrite, check = true) => {
        ...
        return {
          ...
          fn: async (req, res, params, parsedUrl, upgradeHead) => {
            const { newUrl, parsedDestination } = (0,
            prepare_destination_1.prepareDestination)({
            ...
            (0, request_meta_1.addRequestMeta)(
              req,
              '_nextDidRewrite',
              newUrl !== req.url
            );
  ...
  async findPageComponents({ pathname, query, params, isAppPath }) {
    let paths = [
      ...
      query.amp
        ? `${
            isAppPath
              ? (0, app_paths_1.normalizeAppPath)(pathname)
              : (0, normalize_page_path_1.normalizePagePath)(pathname)
          }.amp`
        : null,
      pathname,
    ].filter(Boolean);
  ...
  getFontManifest() {
    return (0, require_1.requireFontManifest)(
      this.serverOptions.computeJs.assets,
      this.distDir,
      this.dir,
      this._isLikeServerless
  ...
  async getFallback(page) {
    ...
    const fullPagePath = (0, path_1.join)(
      this.serverDistDir,
      'pages',
      `${pagePath}.html`
    );
    return (0, require_1.readAssetFileAsString)(
      this.serverOptions.computeJs.assets,
      fullPagePath,
      this.dir
    );
  ...
  generateRoutes() {
    ...
    const fsRoutes = [
      ...this.generateFsStaticRoutes(),
      {
        ...
        fn: async (req, res, params, _parsedUrl) => {
          ...
          pathname = (0, get_route_from_asset_path_1.default)(
            pathname,
            '.json'
          );
          ...
          if (this.nextConfig.i18n) {
            ...
            normalize_locale_path_1.normalizeLocalePath)(
              pathname,
              this.nextConfig.i18n.locales
            );
            const { defaultLocale } =
              (0, detect_domain_locale_1.detectDomainLocale)(
    ...
    const headers = this.minimalMode
      ...
      : this.customRoutes.headers.map((rule) =>
          (0, server_route_utils_1.createHeaderRoute)({
            rule,
            restrictedRedirectPaths,
          })
        );
    const redirects = this.minimalMode
      ...
      : this.customRoutes.redirects.map((rule) =>
          (0, server_route_utils_1.createRedirectRoute)({
            rule,
            restrictedRedirectPaths,
          })
        );
    ...
    const catchAllRoute = {
      ...
      fn: async (req, res, _params, parsedUrl) => {
        ...
        try {
          ...
          if (
            err instanceof base_server_1.NoFallbackError &&
            bubbleNoFallback
          ) {
            return {
              finished: false,
  ...
  async handleApiRequest(req, res, pathname, query) {
    ...
    let params;
    let pageFound =
      !(0, utils_3.isDynamicRoute)(page) && (await this.hasPage(page));
  ...
  getCacheFilesystem() {
    return {
      ...
      readFileSync: (f) =>
        (0, require_1.readAssetFileAsString)(
          this.serverOptions.computeJs.assets,
          f,
          this.dir
        ),
      ...
      mkdir: (dir) =>
        ...
        Promise.resolve(),
      stat: (f) => Promise.resolve({ mtime: NextComputeJsServer._mtime }),
    };
  }
  ...
  getPrerenderManifest() {
    ...
    const manifestFile = (0, path_1.join)(
      this.distDir,
      constants_1.PRERENDER_MANIFEST
    );
    const manifest = (0, require_1.readAssetManifest)(
      this.serverOptions.computeJs.assets,
      manifestFile,
      this.dir
    );
  ...
  getRoutesManifest() {
    const routesManifestFile = (0, path_1.join)(
      this.distDir,
      constants_1.ROUTES_MANIFEST
    );
    return (0, require_1.readAssetManifest)(
      this.serverOptions.computeJs.assets,
      routesManifestFile,
  ...
  attachRequestMeta(req, parsedUrl) {
    ...
    (0, request_meta_1.addRequestMeta)(req, '__NEXT_INIT_QUERY', {
      ...parsedUrl.query,
    });
    ...
    (0, request_meta_1.addRequestMeta)(
      ...
      (0, body_streams_1.getClonableBody)(req.body)
    );
  }
