...
import { createHeaderRoute, createRedirectRoute, getCustomRoute, stringifyQuery } from 'next/dist/server/server-route-utils';
...
import ComputeJsResponseCache from './response-cache';
...
export default class NextComputeJsServer extends BaseServer {
  constructor(options) {
    ...
    loadComponents(this.serverOptions.computeJs.assets, this.distDir, '/_document', this.dir, this._isLikeServerless, false, false).catch(
      () => {}
    );

    loadComponents(this.serverOptions.computeJs.assets, this.distDir, '/_app', this.dir, this._isLikeServerless, false, false).catch(
      () => {}
    );
  ...
  getFilesystemPaths() {
    ...
    let userFilesPublic = [];
    if (this.publicDir) {
      userFilesPublic = assetDirectory(this.serverOptions.computeJs.assets, join(this.dir, CLIENT_PUBLIC_FILES_PATH), this.dir);
    }
  ...
  generateRoutes() {
    ...
    const headers = this.minimalMode ? [] : this.customRoutes.headers.map((rule) => createHeaderRoute({ rule, restrictedRedirectPaths }));
