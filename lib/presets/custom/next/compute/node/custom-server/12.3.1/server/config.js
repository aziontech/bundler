...
import { CONFIG_FILES } from 'next/constants.js';
...
import { execOnce } from 'next/dist/shared/lib/utils.js';
import {
  imageConfigDefault,
  VALID_LOADERS,
...
function assignDefaults(userConfig) {
  ...
  if (result?.images) {
    ...
    if (images.deviceSizes) {
      ...
      const invalid = deviceSizes.filter(
        (d) => typeof d !== 'number' || d < 1 || d > 10000
      );
    ...
    if (images.imageSizes) {
      ...
      const invalid = imageSizes.filter(
        (d) => typeof d !== 'number' || d < 1 || d > 10000
      );
 ...
 * @param customConfig
 */
export async function loadConfig(phase, assets, dir, customConfig) {
  ...
  if (path?.length) {
    ...
    if (validateResult.errors) {
      ...
      const {
        AggregateAjvError,
      } = require('next/dist/compiled/@segment/ajv-human-errors');
