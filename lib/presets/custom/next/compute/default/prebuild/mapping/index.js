 ...
 * @returns {boolean} true if validRuntime, validEntrypoint and validVersion are true
 */
function isVcConfigValid(vcConfig) {
  ...
  // valid framework versions for runtimes
  const validNextNodeVersions = ['12.3.1', '12.3.4'];
  const validNextEdgeVersions = [
    '12.3.1',
    '12.3.4',
    '13.0.0',
    '13.4.8',
  ...
  if (isEdge) {
    ...
    validVersion = validNextEdgeVersions.includes(vcConfig.framework.version);
  } else if (isNode) {
    validEntrypoint = vcConfig.handler !== undefined;
    validVersion = validNextNodeVersions.includes(vcConfig.framework.version);
  }

  return validRuntime && validEntrypoint && validVersion;
}
...
export async function mapAndAdaptFunctions(
  ...
  const pathsToIgnore = [
    '.vercel/output/functions/favicon.ico.func/.vc-config.json',
  ];
  const vcConfigPaths = glob.sync(
    '.vercel/output/functions/**/.vc-config.json',
    { ignore: pathsToIgnore },
  );
  let vcConfigObjects = vcConfigPaths.map((file) => {
    return {
      path: file,
