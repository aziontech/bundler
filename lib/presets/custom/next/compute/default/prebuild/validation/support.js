...
export function validateSupport(version, runtimes, framework = 'nextjs') {
  ...
  // check if the path version 12.3.x
  if (VERSION_RUNTIME_MAP_SUPPORT.has(versionMinorWithFramework)) {
    result.valid = getAllowedRuntimes(versionMinorWithFramework);
  }
  // check if the path version 12.3.1
  else if (VERSION_RUNTIME_MAP_SUPPORT.has(versionWithFramework)) {
