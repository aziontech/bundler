  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      if (fileStat.isDirectory()) {
        ...
        feedback.info(`Moved ${filePath} to ${newHTMLPath}`);
        ...
        if (files.includes(correspondingTXTFile)) {
          ...
          feedback.info(`Moved ${correspondingTXTPath} to ${newTXTPath}`);
        }
      }
    }),
  );
...
async function prebuild() {
  try {
    feedback.info('Starting Next.js build process...');
    await exec(`${packageManager} run build`, true);

    feedback.info('Adapting Next.js build output...');
    await moveFiles(`${process.cwd()}/.edge/statics`);

    feedback.success('Next.js build adaptation completed successfully.');
  } catch (error) {
    feedback.error('Error occurred during Next.js build adaptation:', error);
  }
}
