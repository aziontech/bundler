...
describe('Utils - generateManifest', () => {
  describe('jsToJson', () => {
    ...
    it('should include the behavior deliver when deliver is true', () => {
      ...
      const result = jsToJson(azionConfigComDeliver);
      expect(result.rules[0].behaviors).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            name: 'deliver',
  ...
  it('should correctly assign order starting from 2 for request and response rules', () => {
    const azionConfig = {
      rules: {
        request: [
          { name: 'First Request Rule', match: '/' },
          { name: 'Second Request Rule', match: '/second' },
        ],
        response: [
          { name: 'First Response Rule', match: '/' },
          { name: 'Second Response Rule', match: '/second' },
        ],
      },
    };
