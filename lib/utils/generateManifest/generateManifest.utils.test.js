...
describe('Utils - generateManifest', () => {
  describe('jsToJson', () => {
    ...
    it('should handle rewrites with req and captured variables correctly', () => {
      const azionConfig = {
        rules: {
          request: [
            {
              ...
              behavior: {
                rewrite: {
                  set: (req) => `${req.host}/script-runner/api/%{captured[1]}`,
                },
              },
            },
          ],
      ...
      expect(result.rules[0].behaviors).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            ...
            target: '${host}/script-runner/api/%{captured[1]}',
          }),
        ]),
      );
    });

    it('should handle rewrites with only req variables correctly', () => {
      const azionConfig = {
        rules: {
          request: [
            {
