...
function processManifestConfig(config) {
  ...
  config?.rules?.request?.forEach((rule) => {
    ...
    if (rule.runFunction) {
      if (typeof rule.runFunction.path !== 'string') {
        throw new Error(
          `Invalid path for runFunction: ${rule.runFunction.path}`,
        );
      }
    ...
    if (rule.deliver) {
      if (typeof rule.deliver !== 'boolean') {
        throw new Error(
          `Invalid type for deliver: ${typeof rule.deliver}. Expected boolean.`,
        );
      }
      cdnRule.behaviors.push({
        rule: 'deliver',
    ...
    if (rule.setCookie) {
      if (typeof rule.setCookie !== 'string') {
        throw new Error(
          `Invalid type for setCookie: ${rule.setCookie}. Expected string.`,
        );
      }
    ...
    if (rule.setHeaders) {
      if (typeof rule.setHeaders !== 'string') {
        throw new Error(
          `Invalid type for setHeaders: ${rule.setHeaders}. Expected string.`,
        );
      }
