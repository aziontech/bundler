...
async function uploadStatics(versionId, basePath, currentPath = '') {
    ...
    if (fileStat.isFile()) {
      ...
      try {
        ...
        if (response.statusText === 'OK') {
          ...
          feedback.success(`Uploaded file: ${filePath}`);
        }
        if (!response.status === 'OK') { throw new Error(response); }
      } catch (error) {
        feedback.error(`Error uploading file: ${filePath}`);
  ...
  feedback.success(`${successUploadCount} static assets successfully uploaded to Azion!`);
