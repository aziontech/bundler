  ...
  const performFetch = async () => {
    try {
      ...
      if (response.status === 404) {
        feedback.interactive.await(`Propagating to ${AzionEdges[currentEdgeIndex]}...`);
        ...
        const remainingEdgesIntervalId = setInterval(() => {
          if (remainingEdgeIndex >= AzionEdges.length) {
            ...
            feedback.interactive.complete('Application has propagated to the edge networks and is ready to be accessed!');
            exec(`open https://${domain}`);
            process.exit(0);
          } else {
            const remainingEdge = AzionEdges[remainingEdgeIndex];
            feedback.interactive.await(`Propagating to ${remainingEdge}...`);
      ...
      feedback.error('Error occurred while watching propagation:', error);
    }
  };
