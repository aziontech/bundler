...
function watchPropagation(domain) {
  // feedback.time('propagation_time');
  ...
  const performFetch = async () => {
    try {
      ...
      if (response.status === 404) {
        ...
        const remainingEdgesIntervalId = setInterval(() => {
          if (remainingEdgeIndex >= AzionEdges.length) {
            ...
            // feedback.timeEnd('propagation_time');
      ...
      debug.error(error);
      feedback.fatal(Messages.platform.propagation.errors.watch_propagation_failed);
    }
  };
