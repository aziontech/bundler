...
async function deploy(applicationName, functionName, domainName) {
  try {
    ...
    feedback.platform.interactive.await(
      `[%d/6] - ${Messages.platform.deploy.info.creating_edge_function}`,
      1,
    );
    ...
    feedback.platform.interactive.await(
      `[%d/6] - ${Messages.platform.deploy.info.creating_edge_application}`,
      2,
    );
    const generatedApplicationName = applicationName || deployId;
    const edgeApplicationId = (
      await createApplication(generatedApplicationName)
    ).id;

    feedback.platform.interactive.await(
      `[%d/6] - ${Messages.platform.deploy.info.activating_edge_function}`,
      3,
    );
    await enableEdgeFunctions(edgeApplicationId);

    feedback.platform.interactive.await(
      `[%d/6] - ${Messages.platform.deploy.info.instantiating_edge_function}`,
      4,
    );
    const functionInstanceId = (
      await instantiateFunction(
    ...
    feedback.platform.interactive.await(
      `[%d/6] - ${Messages.platform.deploy.info.creating_rule_engine}`,
      5,
    );
    await setFunctionAsDefaultRule(edgeApplicationId, functionInstanceId);

    feedback.platform.interactive.await(
      `[%d/6] - ${Messages.platform.deploy.info.creating_domain}`,
      6,
    );
    const generatedDomainName = domainName || deployId;
    const domain = (await createDomain(generatedDomainName, edgeApplicationId))
      .domain_name;
    feedback.platform.interactive.complete(
      Messages.platform.deploy.success.deploy_finished,
    );
