import { feedback, generateTimestamp } from '#utils';
...
async function deploy(applicationName, functionName, domainName) {
  try {
    ...
    feedback.success('Worker successfully uploaded to Edge!');
    ...
    feedback.success('Application deployed successfully.');

    return domain;
  } catch (error) {
    feedback.error('An error occurred during deployment:', error);
    throw error;
  }
}
