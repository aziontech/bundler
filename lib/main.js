 ...
 * @example
 * const originalOptions = { 'use-node-polyfills': true, 'only-manifest': false };
 * const convertedOptions = convertOptions(originalOptions);
 * // Result: { useNodePolyfills: true, onlyManifest: false }
 */
...
function startVulcanProgram() {
  ...
  program
    ...
    .option(
      '--use-node-polyfills [boolean]',
      'Use node polyfills in build. Use --use-node-polyfills or --use-node-polyfills=true to enable, --use-node-polyfills=false to disable',
    )
    .option('--only-manifest', 'Process just the azion.config.js')
    .option(
      '--use-own-worker [boolean]',
      'Indicates that the constructed code inserts its own worker expression. Use --use-own-worker or --use-own-worker=true to enable, --use-own-worker=false to disable',
    )
    .option(
      '--firewall',
