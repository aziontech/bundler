...
function startVulcanProgram() {
  ...
  program
    ...
    .action(async (options) => {
      const { build } = await import('#commands');
      await build(options);
    });
  ...
  program
    ...
    .action(async (file, options) => {
      const { dev } = await import('#commands');
  ...
  program
    ...
    .action(async (command) => {
      const { presets } = await import('#commands');
      await presets(command);
    });
  ...
  if (debugEnabled) {
    program
      ...
      .action(async (type, id, options) => {
        const { logs } = await import('#commands');
        await logs(type, id, options);
      });
    ...
    program
      ...
      .action(async (options) => {
        const { auth } = await import('#commands');
        await auth(options);
      });
    ...
    program
      ...
      .action(async () => {
        const { storage } = await import('#commands');
        await storage();
      });
    ...
    program
      ...
      .action(async () => {
        const { deploy } = await import('#commands');
        await deploy();
      });
  }
