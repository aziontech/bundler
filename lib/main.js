...
function setVulcanEnvironment() {
  ...
  if (!['production', 'stage', 'local'].includes(ENV)) {
    feedback.error('Invalid environment. Please set ENV to either production, stage, or local.');
    process.exit(1);
  } else {
  ...
  program
    ...
    .action(async (type, id, options) => {
      ...
      if (!['function', 'application'].includes(type)) {
        feedback.error('Invalid log type. Please specify either "function" or "application".');
        return;
      }

      if (type === 'function') { functions.actions.showFunctionLogs(id, watch); }
      if (type === 'application') { feedback.info('Logs for applications are not yet supported. Please specify "function" instead.'); }
    });
  program
    .command('build')
    .description('Build a project for edge deployment')
    .option('--preset <type>', 'Preset of build target (e.g., vue, next, js)', 'javascript')
...
try {
  ...
  feedback.error('An error occurred:', err);
}
