...
import vulcan from '../../env/bundler.js';
import {
  createDotEnvFile,
...
class Dispatcher {
   ...
   * @param {string}[config.bundler] - The name of the Bundler you want to use (Esbuild or Webpack)
   ...
   * @param {bundlerConfig} config - The configuration object.
   * @returns {bundlerConfig} - Returns the configured build object.
   */
  static configureBuild(currentConfig, config) {
    ...
    const tempbundlerEntryPath = join(currentDir, tempEntryFile);

    buildConfig.entry = tempbundlerEntryPath;
    buildConfig.output = globalThis.bundler?.buildProd
      ? '.edge/worker.js'
      : '.edge/worker.dev.js';
   ...
   * @param {bundlerConfig} buildConfig - The build configuration object.
  ...
  static async runPrebuild(context, buildConfig) {
    ...
    if (injectionDirs && injectionDirs.length > 0) {
      ...
      codeToInjectInHandler.onBanner += `globalThis.bundler = {}; globalThis.bundler.FS_PATH_PREFIX_TO_REMOVE = '${prefix}';\n${content}`;
    }
    ...
    prebuildContext.bundlerPlugins = bundlerPlugins;
   ...
   * @param {string} bundlerCurrent - The bundler current e.g esbuild.
   * @param {bundlerConfig} buildConfig - The build configuration object.
   * @returns {Promise<void>} - A promise that resolves when the build process is completed.
   */
  static async executeBuild(originalEntry, bundlerCurrent, buildConfig) {
    const bundlerSelected = bundlerCurrent || buildConfig.bundler;
    let bundler;

    switch (bundlerSelected) {
      case 'webpack':
        bundler = createAzionWebpackConfig(buildConfig);
        await executeWebpackBuild(bundler);
        break;
      case 'esbuild':
        bundler = createAzionESBuildConfig(buildConfig);
        await executeESBuildBuild(bundler);
        break;
      default:
        bundler = createAzionWebpackConfig(buildConfig);
        await executeWebpackBuild(bundler);
  ...
  async run(processOnlyManifst) {
    ...
    try {
      if (!processOnlyManifst) {
        ...
        await Dispatcher.executeBuild(this.entry, this.bundler, buildConfig);
        ...
        if (this.polyfills) {
          ...
          const outPath = globalThis.bundler.buildProd
            ? 'worker.js'
            : 'worker.dev.js';
          ...
          let codeToAdd = '';
          if (globalThis.bundler.buildProd) {
