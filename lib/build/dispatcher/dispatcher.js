...
async function loadBuildContext(preset, entry, mode) {
  const vulcanRootPath = await packageDirectory();
  const examplesFolderPath = `${vulcanRootPath}/examples`;
  ...
  // set preset context
  const libBasePath = getAbsoluteLibDirPath();

  let configFilePath;
  let prebuildFilePath;
  let handlerFilePath;

  const modePath = join(libBasePath, 'presets', 'frameworks', preset, mode);
  ...
  if (!existsSync(modePath)) {
    configFilePath = join(libBasePath, 'presets', 'frameworks', preset, 'config.js');
    prebuildFilePath = join(libBasePath, 'presets', 'frameworks', preset, 'prebuild.js');
    handlerFilePath = join(libBasePath, 'presets', 'frameworks', preset, 'handler.js');
  }
  ...
  // inside the ./vulcan/examples folder we don't need to change the alias
  if (!process.cwd().startsWith(examplesFolderPath)) {
    const edgehooks = await getAliasPath('edge');
    newEntryContent = newEntryContent.replace('#edge', edgehooks);
  }
