...
function getAliasPath(alias) {
  const clientFilePath = new URL(import.meta.url).pathname;
  const clientDirectory = dirname(clientFilePath);
  const projectRoot = resolve(clientDirectory, '../../../');
  const packageJsonPath = resolve(projectRoot, 'package.json');
  const packageJsonContent = readFileSync(packageJsonPath, 'utf8');
  const packageJson = JSON.parse(packageJsonContent);
  let path = packageJson.imports[`#${alias}`];
  path = path.replace('./', '../node_modules/vulcan/');
  return path;
}
...
async function loadBuildContext(preset, entry, mode) {
  ...
  const edgehooks = getAliasPath('edge');
