...
class Webpack extends BundlerBase {
  ...
  async applyConfig(config) {
    ...
    if (this.builderConfig.contentToInject) {
      ...
      const bannerPluginIndex = modifyConfig.plugins.findIndex(
        (plugin) => plugin instanceof webpack.BannerPlugin,
      );

      if (bannerPluginIndex !== -1) {
        const oldContent =
          modifyConfig.plugins[bannerPluginIndex].options.banner;
        const pluginToRemove = modifyConfig.plugins[bannerPluginIndex];
        modifyConfig.plugins = modifyConfig.plugins.filter(
          (plugin) => plugin !== pluginToRemove,
        );
        modifyConfig.plugins.push(
